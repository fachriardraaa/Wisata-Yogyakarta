...
const LandingTrip = () => {
  ...
  const timer = setInterval(() => {
    setIndex((prev) => (prev + 1) % tripData.length);
  }, 4000);
  ...
  const getVisibleItems = () => {
    const items = [];
    for (let i = -1; i <= 1; i++) {
      let idx = (index + i + tripData.length) % tripData.length;
      items.push({ ...tripData[idx], position: i });
    }
    return items;
  };
                  ...
                  >
                    ...
                    <div className="card-info-landing">
                      ...
                      <div className="price-row-landing">
                        ...
                        <span className="price-val">Rp {(item?.harga || 0).toLocaleString("id-ID")}</span>
                      </div>
                      
                      {/* KODE BARU (Aman dari error undefined/null) */}
